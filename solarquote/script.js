...
const STORAGE_KEY = "regent-solar-funnel-v1";
        ...
        function money(value) {
            return "\u20a6" + roundEstimate(value).toLocaleString("en-NG");
        }

        function moneyRange(low, high) {
            const roundedLow = roundEstimate(low);
            const roundedHigh = roundEstimate(high);
...
saveProgress();
    ...
    return Promise.race([request, timeout]).then(() => {
        ...
        function buildQuoteExportNode() {
            const resultStep = steps[steps.length - 1];
            const clone = resultStep.cloneNode(true);
            clone.removeAttribute("id");
            clone.classList.add("active");

            // Strip anything that isn't part of the quote itself.
            clone.querySelectorAll(".save-quote-row, .cta-band, .downsell-actions").forEach((el) => el.remove());

            // Only keep the phased-scale card in the exported image if it was actually shown.
            const downsellBox = document.getElementById("downsellBox");
            if (!downsellBox || !downsellBox.classList.contains("active")) {
                const downsellClone = clone.querySelector("#downsellBox");
                if (downsellClone) {
                    downsellClone.remove();
                }
            }

            const brandRow = document.createElement("div");
            brandRow.className = "quote-export-brand";
            brandRow.innerHTML = "<strong>Regent Media</strong><span>Solar Growth Estimate</span>";

            const stage = document.createElement("div");
            stage.className = "question-stage quote-export-stage";
            stage.appendChild(brandRow);
            stage.appendChild(clone);

            const panel = document.createElement("div");
            panel.className = "panel quote-export-panel";
            panel.appendChild(stage);

            const wrapper = document.createElement("div");
            wrapper.className = "quote-export-wrapper";
        ...
        function saveQuoteAsImage() {
            if (typeof html2canvas !== "function") {
                alert("Image export isn't available right now -- please try again in a moment.");
                return;
            }

            const saveBtn = document.getElementById("saveQuoteImage");
            const originalLabel = saveBtn.textContent;
            saveBtn.disabled = true;
            saveBtn.textContent = "Preparing image...";

            const wrapper = buildQuoteExportNode();
            document.body.appendChild(wrapper);

            html2canvas(wrapper, { backgroundColor: "#0f0f10", scale: 2 })
                .then((canvas) => {
                    const link = document.createElement("a");
                    link.download = "regent-media-growth-estimate.png";
                    link.href = canvas.toDataURL("image/png");
                    link.click();
                })
                .catch(() => {
                    alert("Couldn't save the image right now -- please try again.");
                })
                .finally(() => {
        ...
        document.getElementById("saveQuoteImage").addEventListener("click", saveQuoteAsImage);
